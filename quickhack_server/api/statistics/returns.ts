// QuickHack API: aggregate-only return statistics for LEADER users.
...
export async function GET(request: NextRequest) {
  ...
  if (isClientRuntime()) {
    return proxyToServer(
      request,
      `/api/statistics/returns${requestUrl.search}`,
      {
        method: "GET",
        contentType: null,
      }
  ...
  return runOperationTrace(
    {
      operationName: "statistics.returns.read",
      source: "HTTP",
      route: "/api/statistics/returns",
      method: "GET",
    },
    async () => {
      const [
        ...
        await Promise.all([
          ...
          import("@/quickhack_server/statistics/return-statistics-service"),
      ...
      try {
        const dispatch = await traceOperationSpan("SERVICE_READ", () =>
          statisticsDispatcher.dispatchStatisticsRead(prisma, {
            ...
            period,
            calculateLive: () =>
              statisticsService.getReturnStatisticsData(prisma, {
                period,
              }),
          })
        );
