...
function candidates(input: ClientNativeExecutionContext) {
  const appRoot = absoluteWindowsPath(input.appRoot, "appRoot");
  const runtimeDir = absoluteWindowsPath(input.runtimeDir, "runtimeDir");
  const values = [
    path.win32.join(runtimeDir, "platform-tools", "adb.exe"),
    path.win32.join(runtimeDir, "adb", "adb.exe"),
    path.win32.join(appRoot, "platform-tools", "adb.exe"),
  ];
  return {
    appRoot,
    values: [...new Map(values.map((value) => [value.toLowerCase(), value])).values()],
  };
}
...
export function createWindowsAdbExecutableResolver(
  ...
  return Object.freeze({
    ...
    async resolve(input) {
      ...
      const cacheKey = `${candidateSet.appRoot}\0${absoluteWindowsPath(
        input.runtimeDir,
        "runtimeDir"
      )}`.toLowerCase();
      ...
      for (const executable of candidateSet.values) {
        ...
        const environment = createChildProcessEnvironment({
          ...
          executableDirectories: [path.win32.dirname(executable)],
        });
        try {
...
export const windowsAdbExecutableResolver =
  createWindowsAdbExecutableResolver();
